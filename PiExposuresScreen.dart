...
class PiExposuresScreen extends StatelessWidget {
  ...
  Future<File> fetchPhotos(BuildContext context, int exposure_id) async {
    final response = await http.get(
        'http://ec2-52-91-107-223.compute-1.amazonaws.com:5000/exposure/pictures?expo_id=' +
            exposure_id.toString());
    if (response.statusCode == 200) {
      ...
      Navigator.push(
          ...
          new MaterialPageRoute(
              builder: (context) =>
                  new ShowExposureScreen(photos, exposures, piName, pis)));
    } else {
                ...
                leading: IconButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => HouseHoldCamerasScreen(pis)),
                      );
                    })),
            body: Padding(
                padding: EdgeInsets.all(10.0),
                child:
                    ListView(children: createButtons(exposures.length, context))
